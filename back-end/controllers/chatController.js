...
export async function chat(req, res){
    ...
    if(!project){
     ...
     // set status to revising and save user promptnimmediately
      project.status = "revising",
      project.messages.push({role: "user", content: prompt, timestamp: new Date()});
      await project.save();

      try {
        //Build compact manifest {pathc+ hash + size } instead of sending all code
        const manifest = buildManifest(project.files);

        //include all files content so the ai can do accurate search/replace
        const relevantFiles = [];
       ...
       console.error(`[AI Revision Error] ${err.messagee}`);
       project.status = "completed";
       await project.save();
