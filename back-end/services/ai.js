...
// Helper: Safely generate structured JSON across diverse LLMs
async function safeGenerateObject({ model, schema, system, prompt, maxRetries = 2 }) {
    const jsonSystemPrompt = `${system}\n\nIMPORTANT: You must return raw, valid JSON only matching the schema. Do NOT include markdown blocks, commentary, or conversational text.`;

    try {
        // Try native structured generation
        ...
        console.warn(`[AI] Standard generateObject failed (${err.message}). Attempting text fallback JSON parse...`);

        // Fallback: Use generateText + JSON extraction for models with weak schema enforcement
...
// Generate project files: plan first, then build files in order
export async function generateProject(prompt, callbacks) {
    console.log(`[AI] Phase 1: Planning file structure for: "${prompt.slice(0, 80)}..."`);
