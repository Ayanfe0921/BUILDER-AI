    ...
    <div className="h-screen overflow-y-scroll text-white font-sans bg-[url('/bg-img.png')] bg-cover bg-center bg-no-repeat">
      ...
      <nav className='sticky top-0 z-10 flex items-center justify-between px-6 py-4'>
        ...
        <div className='flex items-center gap-4 text-sm font-medium text-zinc-300'>
          <span>{user?.name}</span>
          <button onClick={logout} className='py-1.5 px-3 border border-white/20 text-white hover:hb-white/10 text-xs rounded-md cursor-pointer bg-transparent'>
            Sign out
          </button>
        </div>
      </nav>
      ...
      {/* Hero */}
      <div className='flex-1 flex flex-col items-center justify-center px-6 pb-20 mt8 xl:mt-28'>
        <div className='w-full max-w-2xl flex flex-col items-center'>
          ...
          <div className='masked-marquee w-full mt-4 max-w-2xl overflow-hidden py-1'>
            <div className='animate-marquee gap-3'>
              {homeTags.map((tag, i) => (
                <button key={i} onClick={() => handleGenerate(tag)} disabled={generatingProject} className='px-4 py-1.5 border rounded-full text-sm text-white bg-white/10 border-white/25 hover:bg-white/20 transition cursor-pointer shrink-0 font-medium'>
                  {tag}
                </button>
              ))}
            </div>
          ...
          {!loadingProjects && projects.length > 0 && (
            <div className="mt-12 w-full">
              ...
              <div className="flex items-center justify-between pb-3 mb-3 border-b border-white/10">
                <p className='text-xs font-medium uppercase text-zinc-100 tracking-widst'>All Projects</p>
              ...
              <div className='space-y-2 max-h-[80vh] overflow-y-auto pr-1'>
                 {projects.map((p)=>(
                  <div key={p._id} className='bg-white/5 border border-white/10 rounded-lg px-4 py-3 flex items-center justify-between group hover:border-white/20 hover:bg-white/10 cursor-pointer bckdrop-blur-md transition-all'
                  ...
                  >
                    ...
                    {/* <div className='flex items-center gap-2'>
                      <button
                        ...
                        className='p-1.5 rounded-md text-zinc-200 hover:text-red-400 hover:bg-white/10 transition-opacity cursor-pointer'
                      // className='p-1.5 rounded-md text-zinc-200 hover:text-red-400 hover:bg-white/10 opacity-10 group-hover:opacity-100 transition-opacity cursor-pointer'
                      >
                        <Trash2Icon size={14} />
                      </button>
                      <ArrowRightIcon size={14} className='text-zinc-200 group-hover:text-white' />
