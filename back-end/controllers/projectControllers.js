...
export async function createProject(req, res){
  ...
  //create projectnin DB immediately with "pending" status
...
async function runBackgroundGeneration(projectId, prompt){
 try {
  console.log(`[Background AI] Starting generation for projects ${projectId}`);
...
//updated projects files (manual edits)
...
export async function getPublicProject(req, res){
 ...
 if(!project.published){
  res.status(403).json({ error: "project ias not published yet"});
  return;
 }
